...
/// 顯示五彩紙屑（慶祝用）動畫：紙屑飄落。
///
/// 此 widget 會填滿可用空間（如同 [SizedBox.expand]）。
///
/// 當 [isStopped] 為 `true` 時，動畫將不會執行。這在 widget 尚不可見時很有用。
/// 提供 [colors] 可以讓動畫在特定情境下看起來更好看。
///
/// 這是 Hemn Chawroka 的 CodePen 的部分移植：
